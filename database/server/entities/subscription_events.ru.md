...
`external_event_id` имеет UNIQUE и поддерживает webhook idempotency.

Nullable FKs с `SET NULL` позволяют сохранять event history после удаления связанного user/subscription.
