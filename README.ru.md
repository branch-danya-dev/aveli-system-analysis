...
**Aveli — не учебная или гипотетическая система. Это мой личный end-to-end продуктовый проект: я придумал продукт, провёл системный анализ, спроектировал решение и реализовал его самостоятельно.**

| Вопрос | Контекст |
|---|---|
| **Откуда появился проект** | Идея продукта, его границы, ограничения и основные пользовательские сценарии были сформированы мной. |
| **Моя роль** | **Автор продукта · Системный аналитик · Проектировщик решения · Разработчик** |
| **За что я отвечал** | Границы продукта и системы, требования и бизнес-правила, владение данными, frontend/backend-архитектура, API-контракты, модель доступа и биллинга, интеграции и реализация. |
| **Как проверялся анализ** | Ключевые аналитические решения были доведены до реальной реализации на Flutter / NestJS / PostgreSQL / RevenueCat. Ограничения фреймворков, провайдеров и кода возвращались обратно в системную модель, если исходные предположения не выдерживали реализации. |
| **Что представляет собой этот репозиторий** | Читабельная база системного знания реализованного продукта, а не выгрузка рабочих заметок или исходного кода приложения. |
| **Текущий результат** | Реализованный продуктовый кейс со стабильной аналитической базой и явными условиями, при которых решения должны быть пересмотрены при развитии продукта. |

В этом проекте не было передачи «аналитик → разработчик»: одна и та же системная модель проходила через анализ, проектирование и реализацию под моей ответственностью.
